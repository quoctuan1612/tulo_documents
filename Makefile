...
db-migrate-up:
	scripts/db_migrate "postgres://localhost:5640/app_development?sslmode=disable&user=postgres" up
	scripts/db_migrate "postgres://localhost:5640/app_test?sslmode=disable&user=postgres" up
	docker-compose run --rm app ./bin/rails db:environment:set RAILS_ENV=test

db-migrate-down:
	scripts/db_migrate "postgres://localhost:5640/app_development?sslmode=disable&user=postgres" down
	scripts/db_migrate "postgres://localhost:5640/app_test?sslmode=disable&user=postgres" down
...
usage:
	@echo usage: [build, db-create, up, up-bridge, down, stop, db-migrate-up, db-migrate-down, db-drop-create, reset, bundle, bundle-update, console, rubocop-fix, rspec, update-pb, attach, seed_fu, init_data, bash]
